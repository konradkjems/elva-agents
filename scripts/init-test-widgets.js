...
async function initTestWidgets() {
  ...
  try {
    ...
    // Insert test widgets
    const result = await collection.insertMany(testWidgets);
    console.log(`Inserted ${result.insertedCount} test widgets`);
    ...
    console.error('Error initializing test widgets:', error);
  } finally {
    await client.close();
  }
}
